...
exports.generateParetoAnalysis = generateParetoAnalysis;
/**
...
const GenerateParetoAnalysisInputSchema = genkit_2.z.object({
    chartDataJson: genkit_2.z.string().describe('The Pareto chart data, in JSON format. Each item has "name" (business line), "value" for the ranked metric, "cumulativePercentage", and a "color" field (SHOULD NOT BE USED by AI for description) based on variable_cost_ratio (变动成本率) and its "vcr" value.'),
    analyzedMetric: genkit_2.z.string().describe('The metric being analyzed in the Pareto chart (e.g., 跟单保费, 总赔款).'),
...
const GenerateParetoAnalysisOutputSchema = genkit_2.z.object({
    summary: genkit_2.z.string().describe('A structured and concise analysis of the Pareto chart in Chinese, using Markdown for emphasis. It should highlight key contributors (the "vital few" typically contributing to 80% of the total), their variable_cost_ratio business status implications, and any significant observations from the "trivial many," providing deep business insights.'),
});
// Exported function to be called by the application
async function generateParetoAnalysis(input) {
    return paretoAnalysisFlow(input);
}
// Define the Prompt for Pareto Analysis
const paretoAnalysisPrompt = genkit_1.ai.definePrompt({
...
const paretoAnalysisFlow = genkit_1.ai.defineFlow({
...
//# sourceMappingURL=generate-pareto-analysis-flow.js.map