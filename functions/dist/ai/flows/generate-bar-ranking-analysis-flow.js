...
exports.generateBarRankingAnalysis = generateBarRankingAnalysis;
/**
...
const GenerateBarRankingAnalysisInputSchema = genkit_2.z.object({
    chartDataJson: genkit_2.z.string().describe('The bar ranking chart data, in JSON format. Each item has a "name" (business line), a value for the ranked metric, vcr (variable_cost_ratio), and a "color" field (SHOULD NOT BE USED by AI for description) based on variable_cost_ratio.'),
    rankedMetric: genkit_2.z.string().describe('The metric being ranked (e.g., 跟单保费, 满期赔付率).'),
...
const GenerateBarRankingAnalysisOutputSchema = genkit_2.z.object({
    summary: genkit_2.z.string().describe('A structured and concise analysis of the bar ranking chart in Chinese. It should highlight top/bottom performers, significant gaps, and consider the variable_cost_ratio-based business status implications of bars and the selected metric, providing deep business insights.'),
});
async function generateBarRankingAnalysis(input) {
    return barRankingAnalysisFlow(input);
}
const barRankingAnalysisPrompt = genkit_1.ai.definePrompt({
...
const barRankingAnalysisFlow = genkit_1.ai.defineFlow({
...
//# sourceMappingURL=generate-bar-ranking-analysis-flow.js.map