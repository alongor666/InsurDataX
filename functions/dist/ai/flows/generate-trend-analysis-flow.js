...
const GenerateTrendAnalysisInputSchema = genkit_2.z.object({
    chartDataJson: genkit_2.z.string().describe('The trend chart data, in JSON format. Each item typically has a "name" (period label) and keys for different business lines or a total, with their values for the selected metric. Each data point for a line may also include a "color" field (SHOULD NOT BE USED by AI for description) based on variable_cost_ratio and its "vcr" value.'),
    selectedMetric: genkit_2.z.string().describe('The primary metric being trended (e.g., 跟单保费, 满期赔付率).'),
    analysisMode: genkit_2.z.string().describe('The analysis mode (e.g., cumulative, periodOverPeriod). "periodOverPeriod" shows the change from the previous period.'),
    currentPeriodLabel: genkit_2.z.string().describe('The label for the most recent period in the trend (e.g., "2025年第22周").'),
...
const GenerateTrendAnalysisOutputSchema = genkit_2.z.object({
    summary: genkit_2.z.string().describe('A structured and concise analysis of the trend chart in Chinese. It should highlight key trends, inflection points, and potential insights relevant to the car insurance business, considering the selected metric, analysis mode, and the business status implications of variable_cost_ratio (derived from vcrColorRules).'),
});
async function generateTrendAnalysis(input) {
    return trendAnalysisFlow(input);
}
const trendAnalysisPrompt = genkit_1.ai.definePrompt({
...
const trendAnalysisFlow = genkit_1.ai.defineFlow({
...
//# sourceMappingURL=generate-trend-analysis-flow.js.map