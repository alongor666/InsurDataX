...
exports.generateBubbleChartAnalysis = generateBubbleChartAnalysis;
/**
...
const GenerateBubbleChartAnalysisInputSchema = genkit_2.z.object({
    chartDataJson: genkit_2.z.string().describe('The bubble chart data, in JSON format. Each item has id, name (business line name), x, y, z values for the selected metrics, vcr (variable_cost_ratio), and a color (SHOULD NOT BE USED by AI for description) based on variable_cost_ratio.'),
    xAxisMetric: genkit_2.z.string().describe('The metric represented on the X-axis (e.g., "跟单保费").'),
    yAxisMetric: genkit_2.z.string().describe('The metric represented on the Y-axis (e.g., "满期赔付率").'),
    bubbleSizeMetric: genkit_2.z.string().describe('The metric represented by the bubble size (e.g., "保单数量").'),
    analysisMode: genkit_2.z.string().describe('The analysis mode (e.g., cumulative, periodOverPeriod).'),
    currentPeriodLabel: genkit_2.z.string().describe('The label for the current data period (e.g., "2025年第22周").'),
...
const GenerateBubbleChartAnalysisOutputSchema = genkit_2.z.object({
    summary: genkit_2.z.string().describe('A structured and concise analysis of the bubble chart in Chinese, using Markdown for emphasis. It should classify business lines (e.g., star, cash cow, problem child, dog based on the three metrics and VCR business status), highlight outliers, clusters, and relationships, considering the variable_cost_ratio-based business status implications and selected metrics.'),
});
async function generateBubbleChartAnalysis(input) {
    return bubbleChartAnalysisFlow(input);
}
const bubbleChartAnalysisPrompt = genkit_1.ai.definePrompt({
...
const bubbleChartAnalysisFlow = genkit_1.ai.defineFlow({
...
//# sourceMappingURL=generate-bubble-chart-analysis-flow.js.map