...
- **#51. 生产构建失败: 类型不匹配 (npm run build Type Mismatch)**
    - **问题**: 应用在本地 `npm run dev` 正常运行，但在执行 `npm run build` 进行生产构建时，因 TypeScript 类型错误而失败。错误指向 `page.tsx`，提示不能将 `null` 值赋给图表数据结构中不允许为 `null` 的字段。
    - **解决方案**: 这是由于生产构建的类型检查比开发环境更严格。通过全面审查 `page.tsx` 中所有为图表（趋势图、排名图、气泡图等）准备数据的逻辑，对所有可能为 `null` 或 `undefined` 的指标值，在使用前通过**空值合并运算符** (`?? 0`) 提供一个默认值 `0`。这确保了传递给图表组件的数据始终符合其严格的类型定义，从而解决了构建失败问题，并提升了应用的健壮性。
