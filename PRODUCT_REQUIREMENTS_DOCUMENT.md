...
*   **场景一：日常业务监控与周报生成**
    ...
    *   选择最新的数据周期作为“当前周期”，并可选择任一周作为“对比周期”（默认为智能环比/同比）。查看KPI看板，了解各项核心指标的达成情况及与所选对比周期的变化。
    ...
    *   他对每个图表单独生成AI分析，获取针对当前图表数据和筛选条件的、结构化的解读（内容会使用Markdown加粗突出重点，并解释VCR颜色背后的业务含义）。
    *   他使用“数据表”视图，核对特定业务类型的原始和聚合数据，数值按全局格式化规则显示。当选择了自定义对比周期时，表格中会显示与该对比周期的变化。
    *   最后，他使用“导出”功能，将当前数据表（已应用筛选、分析模式和对比周期）导出为CSV文件，作为周报素材。
*   **场景二：管理层快速概览**
    *   王总在晨会前打开应用，默认进入KPI看板，迅速浏览关键指标的达成状态和风险高亮（对比数据为默认的环比/同比）。
...
*   **F-KPI-007**: “自主系数”在多选业务类型或合计时，应显示为“-”或不适用。其对比变化值始终显示为“-”。

### 4.2. 趋势分析 (F-TREND)
*   **F-TREND-001**: 以折线图形式可视化所选指标在连续数据周期（默认最近12周，可追溯至最早有数据的周期）内的趋势。趋势图本身不直接受“自定义对比周期”选择器的影响，它总是展示时间序列。
...
*   **F-TREND-005**: 支持多条业务线（或合计线）在同一图表中对比展示。
*   **F-TREND-006**: 图表应包含图例，清晰标识每条线的含义。
*   **F-TREND-007**: 线条颜色根据其对应业务线（或合计）在该周期的变动成本率动态变化。
...
### 4.7. AI 智能分析
*   **F-AI-001: 总体业务摘要**
    *   AI模型根据当前筛选条件（数据源、当前周期、自定义对比周期（如有）、业务类型、分析模式）下的KPI数据和主要业务线表现，生成结构化摘要。输出使用Markdown加粗突出重点。
*   **F-AI-002: 图表AI分析**
    *   AI模型根据当前图表显示的数据（基于当前周期）和相关上下文生成结构化解读。输出使用Markdown加粗突出重点，并解释VCR颜色背后的业务含义。
...
*   权限管理
*   数据库集成深化
*   ~~用户自定义对比期选择~~ (已移至核心功能 F-CTRL-002B)
...
| 1.3  | 2024-05-24 | AI项目大师 | 优化排名图颜色、高度、标签格式、指标范围；气泡图移除图例。 |
| 1.4  | 2024-05-24 | AI项目大师 | 优化排名图数据标签和X轴刻度格式，Y轴业务名称显示，X轴动态单位。 |
| 1.5  | 2024-05-24 | AI项目大师 | AI分析输出结构化，响应用户筛选。修复条形图精细化VCR颜色。 |
| 1.6  | 2024-05-24 | AI项目大师 | 应用全局数值显示格式化规则，统一各处指标显示。       |
| 1.7  | 2024-05-24 | AI项目大师 | 精细化全局数值格式（千位分隔符）。KPI“保费”变化颜色逻辑更新。“自主系数”环比显示“-”。“自定义对比期”移至未来展望。 |
| 1.8  | 2024-05-24 | AI项目大师 | 添加数据源切换功能 (JSON/DB)。将“自定义对比期选择”提升为核心功能需求(F-CTRL-002B)，并更新相关用户场景和KPI显示逻辑。 |
