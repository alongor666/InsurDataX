...
*   **F-AUTH-002: 凭证输入**: 登录页面包含邮箱和密码输入框。
...
*   F-CTRL-003: 业务类型筛选 (同V3.6.0最终实现)
*   F-CTRL-004: 数据导出
*   F-CTRL-005: 视图切换 (应用头部按钮)

### 4.10. AI 智能分析 (已禁用)
*   **F-AI-001: 全局禁用**: 所有AI相关的摘要和图表分析功能均已在前端移除或禁用。
*   **F-AI-002: UI提示**: 应用界面会在原AI功能区域（如KPI看板下方）提示用户AI功能已禁用。
*   **F-AI-003: 后端保留**: 后端Firebase Function (`generateAiSummaryProxy`) 和Genkit flow代码保留，但前端不再调用，因此相关API密钥当前不在前端或Function运行时直接使用。
...
## 8. 未来展望 (可选)
*   (其他无变化)
*   AI功能未来可能根据需求重新评估并启用，届时需要将数据源迁移到受保护的后端Function，并对AI代理Function进行认证保护。
