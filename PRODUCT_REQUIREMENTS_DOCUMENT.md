...
*   场景一：日常业务监控与周报生成
    ...
    *   他选择周期、业务类型。
    *   他切换视图（趋势、气泡、排名、占比、帕累托、数据表）。在每个图表视图的下方，都有一个独立的AI分析模块，他可以点击相应的按钮，前端调用Firebase Function，Function调用Genkit flow生成针对当前图表数据的分析，结果显示在该图表下方的AI摘要区域。
    *   他使用“导出”功能导出数据。
*   场景二：管理层快速概览
    *   王总打开应用，首先浏览KPI看板及其下方的AI生成的总体业务摘要。然后可能切换到其他图表视图，并按需触发该图表的AI分析以获取更具体的解读。
*   场景三：专项问题分析 (特定周期对比)
...
### 4.1. KPI 看板 (F-KPI)
*   (同V3.1.0，其下方会包含AI智能总体业务摘要区域，通过应用头部的AI按钮触发)

### 4.2. 趋势分析 (F-TREND)
*   (同V3.1.0，并在其下方包含独立的AI图表分析功能模块和触发按钮)
...
| :---- | :--------- | :------- | :----------------------------------------------------------------------- |
| ...   | ...        | ...      | ...                                                                      |
| 3.2.0 | 2024-05-31 | AI项目大师 | AI摘要整合: 将AI智能业务摘要固定到KPI看板下方，取消所有其他图表独立的AI分析模块和按钮。 |
