...
*   F-KPI-002: 每个KPI卡片应清晰显示指标名称、当前值（按全局格式化规则显示，不带单位，百分比带%）。
    ...
    *   显示与**对比周期**的变化：同时显示**绝对值变化**和**百分比/百分点变化**。此对比变化不再包含周期标签。箭头物理方向始终表示数值增减（上增下减），颜色（红/绿/灰）表示业务含义好坏。
...
| :--- | :--------- | :------- | :------------------------------------- |
| ...  | ...        | ...      | ...                                    |
| 2.9  | 2024-05-29 | AI项目大师 | 再次优化业务类型筛选器：顶部增加“全选/反选/清除”，每个业务类型条目可勾选并有“仅选此项”按钮，底部增加“确认/取消”按钮以支持部分操作的延迟生效。 |
