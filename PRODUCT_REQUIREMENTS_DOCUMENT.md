...
*   **场景一：日常业务监控与周报生成**
    ...
    *   他使用“水平条形图排名”，选择指标对各业务类型进行排名，条形颜色根据VCR动态变化，数据标签为整数（金额单位为“万”）。
...
### 4.4. 水平条形图排名 (F-RANK)
*   **F-RANK-001**: 使用水平条形图，根据用户选择的单个指标对各业务类型进行排名。图表应有足够的显示高度（例如 `h-[450px]`）。
*   **F-RANK-002**: 用户可从预设的指标列表（包含KPI看板核心指标，排除不适宜排名的）中选择要排名的指标。
*   **F-RANK-003**: 条形图按指标值降序排列。
*   **F-RANK-004**: 每个条形代表一个独立的业务类型，并显示其名称和指标值。
    *   **数据标签**: 条形顶端的数据标签应显示为整数。
    *   **单位统一**: 金额类指标的标签单位统一为“万”（例如 "123 万"）。百分比类标签显示为整数百分比 (例如 "68%")。件数类标签显示为整数件数 (例如 "123 件")。
    *   标签应完整显示，可能需要调整图表右边距。
*   **F-RANK-005**: X轴表示指标值，刻度应适配数据，金额类以“万”、“千万”、“亿”等易读单位显示。Y轴显示业务类型名称。
...
| 1.3  | 2024-05-24 | AI项目大师 | 优化排名图：颜色与VCR挂钩、增加高度、统一标签格式、扩展指标；更新气泡图移除图例。 |
