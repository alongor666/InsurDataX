...
*   场景一：日常业务监控与周报生成
    ...
    *   他可根据需要筛选特定的一个或多个业务类型进行分析。业务类型筛选器支持多选、全选、反选，并为每个业务类型提供“只选此项”和“勾选所有其他项”的快捷操作。
...
*   场景三：专项问题分析 (特定周期对比)
    ...
    *   李明接到指示，打开应用，选择“非营业客车新车”业务类型（可使用“只选此项”快捷操作），将“当前周期”设为W22，将“对比周期”设为W20。他可以先查看“累计数据”模式下的KPI和趋势，然后切换到“当周发生额分析”模式。
...
*   F-KPI-002: 每个KPI卡片应清晰显示指标名称、当前值（按全局格式化规则显示，不带单位，百分比带%）。
    ...
    *   显示与**对比周期**的变化：同时显示**绝对值变化**和**百分比/百分点变化**。此对比变化不再包含周期标签。箭头方向始终表示数值增减（上增下减），颜色（红/绿/灰）表示业务含义好坏。
...
*   F-CTRL-003: 业务类型筛选
    *   支持多选、全选、反选。
    *   为每个业务类型条目提供一个子菜单，包含以下快捷操作：
        *   **只选此项 (清空其他)**: 仅勾选当前业务类型，并取消勾选其他所有业务类型。
        *   **勾选所有其他项**: 勾选除当前业务类型外的所有其他业务类型，并取消勾选当前业务类型。
