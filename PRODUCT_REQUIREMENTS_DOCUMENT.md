...
**版本**: 5.0.0 (As-Built: 客户端直连Firestore安全架构)
**最后更新日期**: (当前日期)
**负责人**: AI项目大师
...
### 1.2. 应用范围与核心价值
本应用是一个**安全、交互式**的车险经营分析仪表盘。其核心价值在于：
...
### 2.1. 技术栈
*   **前端**:
    *   **框架**: Next.js (App Router, 配置为静态导出 `output: "export"`)
    *   **语言**: TypeScript
    *   **UI组件**: ShadCN UI
    *   **样式**: Tailwind CSS
    *   **图表**: Recharts
...
在完成本地开发与测试后，您可以将应用部署到 Firebase Hosting，使其可以公开访问。

1.  **构建静态应用**:
    *   在项目根目录运行以下命令。这将触发 Next.js 的静态导出流程，将整个应用打包为纯静态文件（HTML, CSS, JS），并存放在根目录的 `out` 文件夹中。
    ```bash
    npm run build
    ```
2.  **部署到 Firebase**:
    *   运行以下命令，将 `out` 文件夹中的内容上传到 Firebase Hosting。
    *   `--only hosting` 参数确保了我们只部署网站内容，而不会尝试部署其他 Firebase 服务（如 Functions）。
    ```bash
    firebase deploy --only hosting
    ```
3.  **访问线上应用**:
    *   部署成功后，Firebase CLI 会在终端中显示您的线上应用URL。您可以点击链接访问。
...
| :---- | :--------- | :------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| ...   | ...        | ...      | ... (保留之前所有记录)                                                                                                                   |
| 4.1.0 | (先前日期) | AI项目大师 | **架构调整**: 由于Blaze计费计划限制，将数据获取改为由前端客户端在认证后直接安全地访问Firestore。 |
| 5.0.0 | (当前日期) | AI项目大师 | **文档重构 (As-Built)**: 全面重写PRD，作为详尽的系统设计与功能说明书。增加了“从0到1复刻指南”，确保任何开发者都能理解并重建项目。 |
