...
*   **PoP (Period over Period)**: 环比，本期与上一期（通常是上一周）的比较。**环比变化值逻辑**：指当前期数值与对比期数值的绝对差额。
...
*   **灵活数据接入**: 支持从不同数据源（如本地文件、数据库）加载数据。

### 2.3. 核心价值
*   **实时性与准确性**：基于最新的周度数据，提供准确的业务指标。
*   **可视化与直观性**：通过丰富的图表和卡片，使复杂数据易于理解。
*   **灵活性与可配置性**：支持分析模式切换、业务类型筛选、数据周期选择、数据源切换，满足不同分析需求。
...
### 3.2. 核心使用场景
*   **场景一：日常业务监控与周报生成**
    *   李明打开应用，选择数据源（默认为JSON，未来可切换至数据库）。
    *   选择最新的数据周期，查看KPI看板，了解各项核心指标的达成情况及环比、同比变化。
...
*   **F-KPI-002**: 每个KPI卡片应清晰显示指标名称、当前值（按全局格式化规则显示，不带单位，百分比带%）、环比变化（百分比和绝对值）、同比变化（百分比和绝对值）。绝对值变化也按全局格式化规则显示（不带单位），百分比变化按X.X%显示，百分点变化按X.X pp显示。
*   **F-KPI-003**: KPI数值应根据预设的风险阈值进行高亮显示（例如，红色表示高风险，橙色表示关注，特定指标卡片边框高亮）。
    *   **“保费”指标的环比/同比变化颜色指示**：根据当前聚合数据的变动成本率（VCR）动态确定。若VCR >= 92%（危险区），保费增长显示为负向（红色）；若VCR < 92%（健康/优秀区），保费增长显示为正向（绿色）。
...
*   **F-KPI-007**: “自主系数”在多选业务类型或合计时，应显示为“-”或不适用。其环比、同比变化值始终显示为“-”。
...
*   **F-TABLE-005**: 环比变化应使用箭头（上升/下降/持平）和颜色（根据指标业务含义决定升红降绿或反之，保费环比颜色根据VCR判断）进行可视化提示。
...
*   **F-CTRL-003: 业务类型筛选**
    ...
    *   选择后，所有看板、图表和数据表的数据源均随之更新。
*   **F-CTRL-004: 数据导出**
    *   提供“导出”按钮，允许用户将当前“数据表显示”视图中的数据（根据当前分析模式和筛选条件）导出为CSV文件。
    *   导出的CSV文件中的数值应为原始计算值（例如，保留4位小数的数字），而不是UI上经过全局格式化规则处理后的显示值。列头应包含单位（如“跟单保费(万元)”）。
    *   导出的文件名应包含当前周期、分析模式和所选业务类型等信息。
*   **F-CTRL-005: 视图切换**
    *   提供导航区域（如标签页或按钮组），允许用户在“KPI看板”、“趋势图”、“气泡图”、“排名图”、“数据表”等不同视图间切换。
*   **F-CTRL-006: 数据源选择**
    *   提供下拉菜单，允许用户在“JSON文件”和“PostgreSQL数据库”（或其他已配置的数据库）之间切换数据源。
    *   切换后，应用将尝试从选定源重新加载所有数据。
    *   如果选择数据库但连接失败或数据为空，应向用户提供清晰的提示信息。
    *   默认为“JSON文件”。（数据库连接和查询逻辑的完整实现是后续任务。）

### 4.7. AI 智能分析
*   **F-AI-001: 总体业务摘要**
    *   提供“AI摘要”按钮，点击后，AI模型根据当前筛选条件（数据源、周期、业务类型、分析模式）下的KPI数据和主要业务线表现，生成一段中文的、结构化的业务绩效亮点、潜在风险和初步洞察的摘要。摘要应包含明确的小标题，如“核心指标解读”、“主要业务线表现”、“主要亮点与机遇”、“潜在风险与关注点”。
    *   摘要应显示在页面显著位置。
    *   生成过程中应有加载提示。
*   **F-AI-002: 图表AI分析**
    *   在“趋势分析”、“对比气泡图”、“水平条形图排名”视图下方，分别提供独立的“生成AI分析”按钮。
    *   点击后，AI模型根据当前图表显示的数据和相关上下文（如所选指标、分析模式、VCR颜色规则、筛选的业务类型、数据源），生成一段针对该图表的、结构化的中文解读。解读应包含明确的小标题，例如针对趋势图的“整体趋势解读”、“关键节点与变化”，针对气泡图的“关键业务线表现解读”、“集群与分布特征”，针对排名图的“头部梯队表现”、“尾部梯队表现”。
    *   分析结果显示在对应图表的下方。
    *   生成过程中应有加载提示。
    *   如果图表无数据显示，则此按钮应禁用。
...
*   **NF-PERF-002**: 数据切换与筛选响应：切换数据周期、分析模式、业务类型或图表指标后，数据和图表更新应在2秒内完成。切换数据源时，响应时间取决于数据源本身，但应有明确加载提示。
...
*   **NF-MAIN-004**: 配置文件（如颜色主题、AI模型配置、数据库连接信息）应易于修改，敏感信息通过环境变量管理。

### 5.5. 安全性
*   **NF-SEC-001**: （针对本地应用）主要关注前端代码安全，避免常见的XSS等注入风险（尽管在本应用场景下风险较低）。
*   **NF-SEC-002**: AI Genkit调用应配置适当的API密钥管理和安全措施。
*   **NF-SEC-003**: 数据库连接凭证不应硬编码在前端代码中，应使用环境变量，并通过后端API（如果需要）安全访问。
...
### 7.1. 数据源
*   **当前主要数据源**: 项目 `public/data/insurance_data_v4.json` 文件。
*   **未来支持数据源**: PostgreSQL 数据库。应用提供切换机制。
*   `insurance_data_v4.json` 文件包含按周期组织的车险业务YTD数据。

### 7.2. 数据结构与计算逻辑
*   详细的数据结构、字段定义、基础计算公式、聚合逻辑、同比环比计算规则，请严格参照项目根目录下的 `FIELD_DICTIONARY_V4.md` 文档。此文档是数据准确性的核心依据。
*   从数据库获取数据时，其结构应被转换为与 `V4PeriodData[]` 兼容的格式，以供现有数据处理逻辑使用。

### 7.3. 数据更新频率
*   JSON文件: 目前假设 `insurance_data_v4.json` 文件会按周进行更新。
*   数据库: 更新频率取决于数据库的ETL策略。
*   应用本身不负责数据的采集和ETL过程，仅消费已提供的数据。
...
*   **数据库集成深化**: 完成PostgreSQL的查询逻辑，实现真正的数据动态加载，并考虑性能优化（如分页、缓存）。
*   **用户自定义对比期选择**：允许用户在进行环比或同比分析时，自由选择作为基准的对比数据周期，而不仅仅是上一期或去年同期。
...
| 1.6  | 2024-05-24 | AI项目大师 | 应用全局数值显示格式化规则（初版），统一各处指标的显示方式。更新KPI、图表、数据表和数据导出相关的格式化需求。 |
| 1.7  | 2024-05-24 | AI项目大师 | 应用更精细的全局数值格式化规则（千位分隔符、小数位精确控制）。更新KPI看板“保费”指标变化颜色逻辑。记录“自主系数”环比显示。添加“自定义对比期”到未来展望。更新术语定义“环比”。 |
| 1.8  | 2024-05-24 | AI项目大师 | 添加数据源切换功能需求 (JSON/PostgreSQL)，更新相关用户场景、数据需求和未来展望。 |
