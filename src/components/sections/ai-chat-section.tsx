    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('AI API returned non-JSON error response for Chat:', errorText);
        }
        throw new Error(errorMessage);
      }

      const result = await response.json();
      const newAiMessage: ConversationEntry = { role: 'assistant', content: result.response };
