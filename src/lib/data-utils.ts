...
import { businessLineIcons } from '@/data/mock-data'; // businessLineIcons might need updating for new types
...
// These base values could be YTD or "actuals" for PoP.
const calculateBaseMetricsForSingleEntry = (entry: V4BusinessDataEntry, isPopMode: boolean = false): AggregatedBusinessMetrics => {
  ...
  // avg_premium_per_policy_json is ALWAYS the YTD value from the JSON entry for that business type and period
  // It's used to derive policy_count, even in PoP mode for the "actual" policy_count.
  ...
  // Use new formulas for marginal contribution
...
export const aggregateAndCalculateMetrics = (
  periodBusinessDataEntries: V4BusinessDataEntry[], // Raw YTD entries for the period & selected business types (or all if none selected)
  analysisMode: AnalysisMode,
  originalYtdEntriesForPeriod: V4BusinessDataEntry[], // Unmodified YTD entries for the current period (for single-line YTD pre-calcs & avg_prem_per_pol)
  previousPeriodYtdEntries?: V4BusinessDataEntry[] // Raw YTD entries for the *previous* period (for PoP calc)
  ...
  if (analysisMode === 'periodOverPeriod' && previousPeriodYtdEntries) {
    dataToProcess = periodBusinessDataEntries.map(currentYtdEntry => {
      ...
      return {
        // Spread the original currentYTDEntry to retain avg_premium_per_policy (YTD value)
        // and avg_commercial_index (YTD value)
        ...currentYtdEntry, 
        ...
        // avg_commercial_index for PoP single line uses current YTD's value
        // avg_premium_per_policy (YTD from currentYtdEntry) is used by calculateBaseMetricsForSingleEntry to derive PoP policy_count
        // Set pre-calculated rates/averages (other than avg_premium_per_policy/avg_commercial_index) to null to force recalculation
        loss_ratio: null, 
        expense_ratio: null,
  ...
  // Calculate metrics for each entry (now either YTD or "actuals" for PoP)
  // For PoP, calculateBaseMetricsForSingleEntry will use the "actual" base amounts but the original YTD avg_premium_per_policy
  // from currentYtdEntry to calculate the "actual" policy_count.
  ...
  // Handle single business type selection
  if (periodBusinessDataEntries.length === 1) {
    ...
    if (analysisMode === 'cumulative') {
      ...
      const result: AggregatedBusinessMetrics = {
        ...singleCalculatedMetrics, // Start with recalculated values as fallback
        ...
        policy_count: Math.round(singleCalculatedMetrics.policy_count), // Derived from YTD premium_written and YTD avg_premium_per_policy
    ...
    // For PoP single line, all metrics are recalculated using "actual" base values
  ...
  return {
    ...
    avg_commercial_index: undefined, // Not applicable for aggregate
...
// Helper to filter business data entries, excluding "合计" type
const filterRawBusinessData = (
  ...
  if (selectedTypes.length === 0) { // If no specific types selected, use all individual lines
...
export const processDataForSelectedPeriod = (
  ...
  const currentAggregatedMetrics = aggregateAndCalculateMetrics(
    ...
    analysisMode,
    originalYtdEntriesForCurrentPeriod, // Pass all original YTD entries for the current period
  ...
  const momAggregatedMetrics = momPeriodFilteredYtdEntriesForComparison 
    ? aggregateAndCalculateMetrics(momPeriodFilteredYtdEntriesForComparison, 'cumulative', momOriginalPeriodData.business_data.filter( bd => bd.business_type.toLowerCase() !== '合计' && bd.business_type.toLowerCase() !== 'total') ) 
    : null;
  ...
  const yoyAggregatedMetrics = yoyPeriodFilteredYtdEntriesForComparison
    ? aggregateAndCalculateMetrics(yoyPeriodFilteredYtdEntriesForComparison, 'cumulative', yoyOriginalPeriodData.business_data.filter( bd => bd.business_type.toLowerCase() !== '合计' && bd.business_type.toLowerCase() !== 'total'))
    : null;
  
  let derivedBusinessLineName: string;
  if (selectedBusinessTypes.length === 1) {
    derivedBusinessLineName = selectedBusinessTypes[0];
  } else if (selectedBusinessTypes.length > 1 && selectedBusinessTypes.length === currentPeriodFilteredYtdEntries.length && currentPeriodFilteredYtdEntries.length < originalYtdEntriesForCurrentPeriod.length) {
    // Multiple types selected, but not all available types
    derivedBusinessLineName = "自定义合计";
  }
  else { // Covers empty (all individual types) or selected all individual types
    derivedBusinessLineName = "合计";
  }
  const businessLineId = derivedBusinessLineName; // For simplicity, use the name as ID for display aggregates
  ...
  const processedEntry: ProcessedDataForPeriod = {
    ...
    icon: businessLineIcons[derivedBusinessLineName] || (derivedBusinessLineName === "合计" || derivedBusinessLineName === "自定义合计" ? Users : ShieldCheck), 
    currentMetrics: currentAggregatedMetrics,
    momMetrics: momAggregatedMetrics,
    yoyMetrics: yoyAggregatedMetrics,
    premium_share: premium_share,
    // These direct fields are for DataTableSection convenience, sourced from currentMetrics
    ...
    // Changes (PoP comparison)
    premium_writtenChange: momAggregatedMetrics ? (currentAggregatedMetrics.premium_written - momAggregatedMetrics.premium_written) / (momAggregatedMetrics.premium_written || 1) * 100 : undefined,
    total_loss_amountChange: momAggregatedMetrics ? (currentAggregatedMetrics.total_loss_amount - momAggregatedMetrics.total_loss_amount) / (momAggregatedMetrics.total_loss_amount || 1) * 100 : undefined,
    policy_countChange: momAggregatedMetrics ? (currentAggregatedMetrics.policy_count - momAggregatedMetrics.policy_count) / (momAggregatedMetrics.policy_count || 1) * 100 : undefined,
    loss_ratioChange: momAggregatedMetrics ? currentAggregatedMetrics.loss_ratio - momAggregatedMetrics.loss_ratio : undefined, // Absolute change in percentage points
    expense_ratioChange: momAggregatedMetrics ? currentAggregatedMetrics.expense_ratio - momAggregatedMetrics.expense_ratio : undefined, // Absolute change in percentage points
  };
...
const calculateChangeAndType = (current?: number | null, previous?: number | null, higherIsBetter: boolean = true): { percent?: number, absolute?: number, type: Kpi['changeType'] } => {
  ...
  if (absolute > 0.0001) type = higherIsBetter ? 'positive' : 'negative'; // Use small epsilon for float comparison
  if (absolute < -0.0001) type = higherIsBetter ? 'negative' : 'positive';
...
export const calculateKpis = (
  ...
  analysisMode?: AnalysisMode, // Added to determine if avg_commercial_index should be shown
  selectedBusinessTypes?: string[] // Added to determine if avg_commercial_index should be shown
): Kpi[] => {
  if (!processedData || processedData.length === 0) return [];
  
  const data = processedData[0]; 
  const current = data.currentMetrics;
  // For KPI changes, we compare current period's value (which could be YTD or PoP based on analysisMode)
  // with the YTD value of the comparison period.
  const momYtd = data.momMetrics; 
  const yoyYtd = data.yoyMetrics; 

  if (!current) return [];

  const formatChange = (changeResult: { percent?: number, absolute?: number, type: Kpi['changeType'] }, unit: '万元' | '元' | '%' | '件' = '%', isRateChange: boolean = false, higherIsBetterForColor: boolean = true) => {
    ...
    if (changeResult.absolute !== undefined) {
        ...
        else if (unit === '%') changeAbsStr = `${changeResult.absolute.toFixed(2)} pp`; // For rate changes (percentage points)
    ...
    // For rates, color depends on whether higher rate is good or bad
    // e.g. Loss Ratio: higherIsBetterForColor = false. Positive absolute change (rate went up) => negative type.
    if (isRateChange) { 
        if (changeResult.absolute !== undefined && Math.abs(changeResult.absolute) > 0.0001) {
  ...
  // Determine if avg_commercial_index should be displayed or N/A
  ...
  if (isSingleSelectedType && analysisMode === 'cumulative' && current.avg_commercial_index !== undefined && current.avg_commercial_index !== null) {
      displayAvgCommercialIndex = current.avg_commercial_index.toFixed(4);
      rawAvgCommercialIndex = current.avg_commercial_index;
  } else if (isSingleSelectedType && analysisMode === 'periodOverPeriod' && current.avg_commercial_index !== undefined && current.avg_commercial_index !== null) {
      // For PoP single line, we show the current YTD avg_commercial_index as per V4 doc
      displayAvgCommercialIndex = current.avg_commercial_index.toFixed(4);
      rawAvgCommercialIndex = current.avg_commercial_index;
  }
  ...
  const kpis: Kpi[] = [
    ...
    { 
      ...
      ...formatChange(varCostRatioMomChg, '%', true, false), // higherIsBetterForColor = false
    ...
    { 
      ...
      ...formatChange(expenseRatioMomChg, '%', true, false), // higherIsBetterForColor = false
    ...
    { 
      ...
      ...formatChange(lossRatioMomChg, '%', true, false), // higherIsBetterForColor = false
    ...
    { 
      ...
      icon: ShieldAlert, // Changed icon to ShieldAlert for "赔款"
    },
    { 
    ...
    { 
      id: 'avg_premium_per_policy', title: '单均保费', value: formatCurrency(current.avg_premium_per_policy, '元'), rawValue: current.avg_premium_per_policy,
       ...formatChange(avgPremPolMomChg, '元', false, true), 
      yoyChange: formatChange(avgPremPolYoyChg, '元', false, true).change, yoyChangeAbsolute: formatChange(avgPremPolYoyChg, '元', false, true).changeAbsolute, yoyChangeType: formatChange(avgPremPolYoyChg, '元', false, true).changeType,
      icon: FileText,
    },
    { 
    ...
    { 
      ...
      ...formatChange(claimFreqMomChg, '%', true, false), // higherIsBetterForColor = false
    ...
    { 
      ...
      icon: DollarSign, // Changed to DollarSign to differentiate from "赔款"
    },
  ];
...
// Function to export data to CSV (can be further developed)
export function exportToCSV(data: ProcessedDataForPeriod[], analysisMode: AnalysisMode, fileName: string = "车险数据导出.csv") {
    ...
    if (analysisMode === 'periodOverPeriod') { // This comparison is vs YTD of previous period
        const momHeaders = [
            "跟单保费环比(%)", "跟单保费环比绝对值(万元)",
            "总赔款环比(%)", "总赔款环比绝对值(万元)",
            "保单数量环比(%)", "保单数量环比绝对值(件)",
            "满期赔付率环比(pp)", "满期赔付率环比绝对值(pp)",
    ...
    const rows = data.map(item => {
        ...
        const momYtd = item.momMetrics; // YTD of the MoM comparison period
        ...
        if (analysisMode === 'periodOverPeriod') { // Actually these changes are MoM YTD changes
            if (momYtd) {
                 const premWrittenChange = calculateChangeAndType(current.premium_written, momYtd.premium_written);
                const lossAmtChange = calculateChangeAndType(current.total_loss_amount, momYtd.total_loss_amount);
                const policyCntChange = calculateChangeAndType(current.policy_count, momYtd.policy_count);
                const lossRatioChange = calculateChangeAndType(current.loss_ratio, momYtd.loss_ratio); // Rate change
                const expenseRatioChange = calculateChangeAndType(current.expense_ratio, momYtd.expense_ratio); // Rate change
                ...
                rowData.push(
                    ...
                    lossRatioChange.absolute?.toFixed(2) || "N/A", lossRatioChange.absolute?.toFixed(2) || "N/A", // For pp change, abs is what matters for table
                    expenseRatioChange.absolute?.toFixed(2) || "N/A", expenseRatioChange.absolute?.toFixed(2) || "N/A"
                );
            } else {
                rowData.push(...Array(10).fill("N/A"));
            }
        }
        return rowData.join(",");
    });
