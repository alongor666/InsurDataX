...
const GenerateBusinessSummaryInputSchema = z.object({
  ...
  filters: z.string().describe('The filters applied to the data, in JSON format. This includes analysisMode, period, comparison period info, selectedBusinessTypes, and vcrColorRules (which explains how variable_cost_ratio maps to colors and business implications).'),
});
...
- 变动成本率颜色规则解读: {{filters.vcrColorRules}} (绿色代表优秀/低风险，蓝色代表健康/中等风险，红色代表警告/高风险)
...
**二、主要业务线剖析 (Key Business Line Performance Analysis):**
   ...
   - **解读颜色信号**：明确指出各主要业务线的“变动成本率”颜色指示（红/蓝/绿）及其所代表的业务含义（例如：“**非营业客车新车** 的变动成本率处于**绿色健康区间**，显示其盈利能力和风险控制良好，是优质业务；而 **XX业务** 变动成本率偏高呈**红色警戒**，虽保费贡献可能较大，但其高成本正侵蚀利润，需重点关注其赔付成本和费用结构。”）。
   - 评价这些业务线对整体业绩和利润池的贡献质量。

**三、关键亮点与战略机遇 (Key Highlights & Strategic Opportunities):**
   - 提炼2-3项基于数据的最显著的积极成果或潜在的战略发展机遇。
   - 例如，是否存在某些业务类型展现出高增长且高效率（低变动成本率）的良好态势？是否有通过成本优化提升边际贡献的明显空间？

**四、潜在风险与核心关切 (Potential Risks & Core Concerns):**
   - 指出2-3项最值得关注的潜在风险或业务痛点。
   - 例如，是否有核心业务线的盈利能力正在恶化？成本控制是否存在普遍性问题？市场竞争是否导致价格压力增大，从而压缩利润空间？
