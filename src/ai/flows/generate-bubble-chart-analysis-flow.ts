...
const GenerateBubbleChartAnalysisInputSchema = z.object({
  chartDataJson: z.string().describe('The bubble chart data, in JSON format. Each item has id, name, x, y, z values, and a color based on variable_cost_ratio.'),
...
const GenerateBubbleChartAnalysisOutputSchema = z.object({
  summary: z.string().describe('A concise analysis of the bubble chart, highlighting outliers, clusters, and relationships between metrics for different business lines in Chinese, considering the VCR-based coloring.'),
});
...
Chart Data (each item includes a 'color' field based on VCR and a 'vcr' field with the actual VCR value):
{{{chartDataJson}}}
