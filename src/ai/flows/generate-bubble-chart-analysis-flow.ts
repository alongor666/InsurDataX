...
const GenerateBubbleChartAnalysisInputSchema = z.object({
  chartDataJson: z.string().describe('The bubble chart data, in JSON format. Each item has id, name (business line name), x, y, z values for the selected metrics, vcr (variable_cost_ratio), and a color based on variable_cost_ratio.'),
...
const GenerateBubbleChartAnalysisOutputSchema = z.object({
  summary: z.string().describe('A structured and concise analysis of the bubble chart in Chinese, using Markdown for emphasis. It should classify business lines (e.g., star, cash cow, problem child, dog based on the three metrics and VCR), highlight outliers, clusters, and relationships, considering the variable_cost_ratio-based coloring implications and selected metrics.'),
});
...
您的分析需结合经典的业务组合矩阵思想（如BCG矩阵），并深刻理解“变动成本率”对业务盈利能力和风险的决定性作用。
...
- 变动成本率颜色规则解读: {{filtersJson.vcrColorRules}} (气泡颜色由“变动成本率”决定：绿色代表经营状况优秀/低风险，蓝色代表健康/中等风险，红色代表警告/高风险)

**气泡图数据 (JSON格式):**
(每个条目包含业务线名称 'name', x/y/z轴指标值, 'vcr' (变动成本率)及其对应的 'color')
{{{chartDataJson}}}
...
**一、关键业务线定位与解读 (Key Business Line Positioning & Interpretation):**
   - 识别出2-3个在图表上表现突出的业务线（例如，**最大的气泡**、**位置最偏离的离群点**、或因其“变动成本率”颜色而**特别值得关注**的业务线）。
   - 对每个选定的业务线：
     - 精确描述其在 **{{{xAxisMetric}}}** 和 **{{{yAxisMetric}}}** 维度上的位置，及其由 **{{{bubbleSizeMetric}}}** 代表的规模。
     - **深刻解读其“变动成本率”颜色所指示的业务健康度和盈利能力**。例如：“**非营业客车新车** (绿色大气泡，位于X高Y低区域) 表现为高**{{{xAxisMetric}}}**(如高保费)、低**{{{yAxisMetric}}}**(如低赔付率)、大规模(**{{{bubbleSizeMetric}}}**)，且其**变动成本率健康（绿色）**，属于典型的‘**明星业务**’或‘**现金牛业务**’，贡献了主要的优质利润。” 或 “**某某货车业务** (红色小气泡，位于X低Y高区域) 的**{{{xAxisMetric}}}** 和 **{{{bubbleSizeMetric}}}** 均不高，但 **{{{yAxisMetric}}}** 偏高，且其**变动成本率处于红色警戒状态**，可能属于‘**瘦狗业务**’或高风险业务，需审慎评估其发展策略。”
     - 量化其关键指标值（例如：“跟单保费 **12460万元**”，“满期赔付率 **55.2%**”，“保单数量 **8500件**”，“变动成本率 **85.0% (蓝色)**”）。

**二、业务组合特征与集群分析 (Portfolio Characteristics & Cluster Analysis):**
   - 描述图表中是否出现明显的业务线集群。这些集群在三个维度指标和“变动成本率”风险等级上有何共同特征？
   - 图表的特定象限（例如，高X高Y，低X高Y等）是否更为密集？这对于整体业务组合的健康度、风险集中度或发展阶段有何指示意义？
   - **尝试从业务组合角度进行归类**：基于选定指标和“变动成本率”，哪些业务线更接近“明星”、“现金牛”、“问题（高增长但高风险/低利润）”或“瘦狗（低增长低利润/高风险）”的特征？
...
**三、指标间深层关系与运营启示 (Deep Metric Relationships & Operational Implications):**
   ...
   - “变动成本率”（及其颜色）如何与这些指标相互作用？例如，规模较大 (高 **{{{bubbleSizeMetric}}}**) 的业务线，其“变动成本率”表现是否普遍更优/更差？或者，特定 **{{{xAxisMetric}}}** / **{{{yAxisMetric}}}** 区间的业务线是否更容易出现高风险的“变动成本率”？这对风险定价或资源分配有何启示？

**四、战略洞察与行动建议 (Strategic Insights & Actionable Recommendations):**
   - 基于以上分析，提炼1-2项最具价值的**战略性洞察**。
   - 针对不同定位的业务线（特别是那些“问题业务”或高风险业务），提出**具体的、可操作的初步建议**，例如优化理赔流程以降低赔付、调整定价策略、加强费用管控、或考虑战略性放弃某些持续亏损业务。
...
- **准确运用“变动成本率”及其颜色指示**作为核心分析工具，并清晰解释其业务含义。
