...
const GenerateTrendAnalysisInputSchema = z.object({
  chartDataJson: z.string().describe('The trend chart data, in JSON format. Each item typically has a "name" (period label) and keys for different business lines or a total, with their values for the selected metric.'),
...
const GenerateTrendAnalysisOutputSchema = z.object({
  summary: z.string().describe('A concise analysis of the trend chart, highlighting key trends, inflection points, and potential insights relevant to the car insurance business in Chinese.'),
});
...
- Any notable patterns across different lines if multiple lines are present in the data.
