...
- **#47. 云函数部署失败: 需要Blaze套餐**
    ...
    - **解决方案**: 向用户解释了原因，并指导用户升级到Blaze套餐。**此问题是导致架构转向#48客户端直连模式和#51 AI架构重构的直接原因。**
...
- **#53. 修复 `bar-chart-ranking-section` 组件的健壮性问题**
    - **问题**: 排名图表的X轴域硬编码为从0开始，无法处理负值；同时Tooltip中存在不安全的类型断言。
    - **解决方案**: 将X轴的 `domain` 属性修改为 `['auto', 'auto']`，使其能动态适应数据范围。重构了Tooltip组件，使用类型安全的中间变量处理可能为null的`vcr`值，消除了潜在的运行时错误。

---

## 五、 AI 功能 (从实现到架构重构) (AI Feature Lifecycle: From Implementation to Architectural Pivot)
*本部分完整追溯AI功能从引入、开发、因架构限制而禁用，到最终通过新架构重新启用的生命周期。*
...
- **#51. (接#37) 通过Next.js API路由重新启用AI功能**
