...
// Define a type for all 16 core metrics available in selectors
export type SelectableMetricKey = Exclude<CoreAggregatedMetricKey, 'expense_amount_raw' | 'avg_commercial_index' | 'policy_count_earned' | 'vcr_color'>;

// Specific subsets for different chart types
export type TrendMetricKey = SelectableMetricKey;
export type RankingMetricKey = SelectableMetricKey;
export type BubbleMetricKey = SelectableMetricKey;

export type ShareChartMetricKey = Extract<SelectableMetricKey, 
  | 'premium_written' 
  | 'premium_earned' 
  | 'total_loss_amount' 
  | 'expense_amount' 
