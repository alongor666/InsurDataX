...
// This file centralizes prompt generation for AI calls made from the client-side.
// It is client-safe and can be imported into any component.

// IMPORTANT: Replace this with your deployed Cloudflare Worker URL from your .env.local file
const OPENROUTER_PROXY_URL = process.env.NEXT_PUBLIC_OPENROUTER_PROXY_URL;

export async function callProxy(prompt: string): Promise<string> {
  if (!OPENROUTER_PROXY_URL) {
    throw new Error('OpenRouter Proxy URL is not configured. Please set NEXT_PUBLIC_OPENROUTER_PROXY_URL in your environment variables.');
  }
...
export const getBusinessSummaryPrompt = (input: any, systemInstruction: string): string => {
  const { data, filters } = JSON.parse(input.data);
...
**您的任务：**
请运用帕累托法则（80/20原则）撰写一份结构清晰的中文分析报告。识别**核心贡献业务线（关键少数）**并评估其盈利质量，概览**其余业务线（次要多数）**并挖掘机会，最后给出**帕累托结构洞察与资源配置建议**。请务必结合**变动成本率**所指示的业务状态进行评估。
